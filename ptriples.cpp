...
/* A simple program to generate primitive pythagorean triples.   */
...
int main( int argc, char * argv[] ) {
  ...
  if ( argc != 3 ) {
    ...
    printf("\n");
    printf("Usage: ptriples c_min c_max\n\n");
    return 1;
  }

  mpz_t c_min;
  mpz_init_set_str( c_min,  argv[1], 10 );

  mpz_t c_max;
  mpz_init_set_str( c_max,  argv[2], 10 );

  if ( mpz_cmp_ui( c_min, 1 ) < 0 ) {
  ...
  for ( mpz_set_ui( n, 1 );  mpz_cmp( n, n_max ) <= 0; mpz_add_ui( n, n, 1 ) ) {
    ...
    for ( ; mpz_cmp( m, m_max ) <= 0; mpz_add_ui( m, m, 2 ) ) {
      ...
      if ( mpz_cmp( c, c_min ) < 0 )
        continue;

      if ( mpz_cmp( c, c_max ) > 0 )
        continue;

      if ( mpz_cmp( a, b ) < 0 )
        AddPTriple( &triples, a, b, c );
      else
        AddPTriple( &triples, b, a, c );
    }
  }
...
void AddPTriple( struct ttable* the_ttable, mpz_t a, mpz_t b, mpz_t c ) {
  ...
  mpz_init_set( the_ttable->triples[index].a, a );
  mpz_init_set( the_ttable->triples[index].b, b );
